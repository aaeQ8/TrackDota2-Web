...
import {FilterBar} from './linked.js';
import {LoadingMatches, NoMoreGames} from './players.js';
    ...
    return (
      <Router forceRefresh={true}>
        <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
          <Link className="navbar-brand" to="/">trackdota2</Link>
          <button className="navbar-toggler" type="button" data-toggle="collapse" onClick={this._onToggleNav}
          ...
          <div className={(this.state.navCollapsed ? 'collapse' : '') + ' navbar-collapse'}
            ...
            <div className="navbar-nav">
              <Link className="nav-item nav-link active" to="/live">Live games</Link>
              <Link className="nav-item nav-link active" to="/recent">Recent Matches</Link>
              <Link className="nav-item nav-link active" to="/players">Players</Link>
              <Link className="nav-item nav-link active" to="/linked">Linked matches</Link>
            </div>
          </div>
        </nav>
        <Switch>
...
export class Matches extends React.Component {
  constructor(props) {
    super(props);
    this.state = {"zero_games_found": null,
                  "items" : null,
                  "items_org": null,
                  "last_key" : null,
                  "fetch_more" : true,
  ...
  fetchLinkedMatches() {
    fetch('https://lb6ojeony6.execute-api.ca-central-1.amazonaws.com/dev/linked')
    ...
    .then(data => { 
      if('body' in data) {
        ...
        this.setState({"zero_games_found": true});
      if("last_key" in data)
        this.setState({"last_key" : data['last_key']});
      else {
  ...
  fetchMoreLinkedMatches() {
    if(this.state.last_key !== null && this.state.fetch_more === true) {
      ...
      fetch('https://lb6ojeony6.execute-api.ca-central-1.amazonaws.com/dev/linked' + last_key)
      .then(response => response.json())
      .then(data => { 
        if (this.state.items !== null) {
          this.setState({"items": this.state.items.concat(data['body']['Responses']['matches'])});
          this.setState({"items_org": this.state.items_org.concat(data['body']['Responses']['matches'])});
  ...
  fetchRecentPlayerMatches() {
    fetch('https://lb6ojeony6.execute-api.ca-central-1.amazonaws.com/dev/player?player_id=' + this.props.player_id)
    .then(response => response.json())
    .then(data => { 
      if('body' in data) {
        this.setState({"items": data['body']['Responses']['matches']});
  ...
  fetchMoreRecentPlayerMatches() {
    if(this.state.last_key !== null && this.state.fetch_more === true) {
      var last_key = "&last_key=" + this.state.last_key;
      fetch('https://lb6ojeony6.execute-api.ca-central-1.amazonaws.com/dev/player?player_id=' + this.props.player_id + last_key)
      .then(response => response.json())
      .then(data => { 
        if (this.state.items !== null) {
          this.setState({"items": this.state.items.concat(data['body']['Responses']['matches'])});
      ...
      if(search_words === ''){
        ...
        this.state.items_org.forEach(element => {
          element.players.forEach(player => {
            if(search_words.toLowerCase().includes(player.player_details.player_name.toLowerCase())){
  ...
  fetchRecentMatches() {
    ...
    fetch('https://lb6ojeony6.execute-api.ca-central-1.amazonaws.com/dev/recent_matches' + last_key)
    .then(response => response.json())
    .then(data => { 
      if (this.state.items !== null) {
        this.setState({"items": this.state.items.concat(data['body']['Items'])});
        this.setState({"items_org": this.state.items_org.concat(data['body']['Items'])});
      ...
      if("LastEvaluatedKey" in data['body'])
        this.setState({"last_key" : data['body']['LastEvaluatedKey']['activate_time_id']});
      else {
        this.setState({"last_key": null});
  ...
  renderHeroIcons(start_index, team) {
    if (this.props.match_json != null) {
      ...
      for (var i = start_index; i < start_index+5; i++) 
        {
          cols.push(<HeroPreviewIcon key={this.props.match_json['players'][i]['player_details']['hero_id']}
            player={this.props.match_json['players'][i]} team={team}/>);
        }
      return cols;
    }
  }
        ...
        <div className="live-portrait row">
          ...
          <div className='text-center col-sm-2'>
            <h5 className = 'score-preview' > 
              {this.props.match_json["rad_score"]} - {this.props.match_json["dire_score"]} 
            </h5>
            <h5 className = 'duration-preview'> 
              {result} 
            </h5>
            <h6 className='duration-preview'> {this.props.match_json['average_mmr']} MMR</h6>
            <h6 className = 'duration-preview'> 
              {time_ago(new Date(parseInt(this.props.match_json["activate_time"]*1000)))} 
            </h6>
            <h5 style ={{'color': winnerColor, 'display': 'inline'}} > {this.props.match_json['winner'].toUpperCase()} Win </h5>
          </div>
          {this.renderHeroIcons(5, 'DIRE')}
          <div className='row-sm-2 p-0 m-0'>
            <PlayerNamesView links={m} players={this.props.match_json['players']}/>
          </div>  
        </div>
      );
  }
...
class HeroPreviewIcon extends React.Component {
  render() {
    return (
      <figure style ={{color:'white'}} className='d-none d-lg-block text-center col-sm-1'>
        <img alt={this.props.player['player_details']['player_name']} 
            className='img-fluid' src={'https://d2zromn1qdgyf0.cloudfront.net/heroes_icons/' 
              + this.props.player['player_details']['hero_id'] + '.png'}/>
        <figcaption className='KDA'>
           <span style={{'color': '#90ee90'}}>{this.props.player['stats']['kills']}</span>
             /<span style={{'color': '#FF7F7F'}}>{this.props.player['stats']['deaths']}</span>/
             <span style={{'color': '#ADD8E6'}}>{this.props.player['stats']['assists']}</span>
           </figcaption>
    ...
    if (this.props.items != null) {
      ...
      for (var i = 0; i < this.props.items.length; i++) {
        if (this.props.items[i] !== 0)
          cols.push(<img alt='item' key = {i} className='img-fluid item-preview-icon p-0 m-0' 
            src={'https://d2zromn1qdgyf0.cloudfront.net/items_icons/' + this.props.items[i] + '.png'}/>);
      }
      return cols;
    }
  }
...
class PlayerNamesView extends React.Component {
  renderYtLinks() {
    ...
    if(this.props.links !== null) {
        ...
        for (var i = 0; i < this.props.links.length; i++){
          yt_links.push(
            <a className='mmr-spec' key={this.props.links[i]} 
                href={youtube_link+this.props.links[i]}> 
              <img alt='yt' style={{'maxHeight':' 32px', 'maxWidth': '32px', 'background': 'transparent'}}
  ...
  renderNames() {
    if(this.props.players !== null) {
        ...
        if (this.props.players[i]['player_details']['is_pro'] === "1") {
          names.push(
            <span key={this.props.players[i]['player_details']['hero_id']}> 
              <img alt='hero'
                  src={'https://d2zromn1qdgyf0.cloudfront.net/heroes_mini_icons/' 
                  + this.props.players[i]['player_details']['hero_id'] + '.png'}/>
                <Link style={{'textDecoration': 'none'}} className='player-name-link' to={'/players/' 
                    + this.props.players[i]['player_details']['player_id']}>
                      <span className='player-name' key={this.props.players[i]['player_details']['player_id']}> 
      ...
      if(names.length === 0)
        return <span> No pro players found </span>;
      return names;
    }
  }
