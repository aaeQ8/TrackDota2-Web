...
import {FilterBarLive} from './linked.js'
...
export default class LiveMatchesList extends React.Component {
  ...
  searchItems(search_words, show_val, sort_val) {
    if(this.state.items_org !== null) {
      if(search_words === ''){
        this.setState({'items': this.state.items_org}, () => this.showItems(show_val,sort_val));
       } else {
        var searched_items = [];
        this.state.items_org.forEach(element => {
          element.players.forEach(player => {
            if('name' in player)
              if(player.name.toLowerCase().startsWith(search_words.toLowerCase())){
                searched_items.push(element);
              }
          });
        });
        this.setState({'items': [...searched_items]}, () => this.showItems(show_val, sort_val));
      }
    }
  }
    ...
    if(this.state.items_org !== null) {
      ...
      if(show_val === 'all') {
        ...
        this.state.items.forEach(element => {
          if(element.deactivate_time === 0)
            items_to_show.push(element);
        });
      ...
      for (var i = start_index; i < start_index+5; i++) 
        {
          ...
          cols.push(<LiveHeroPreviewIcon key={this.props.match_data['players'][i]['account_id']}
            player={this.props.match_data['players'][i]} team={team}/>);
        }
      return cols;
    }
  }
        ...
        <div className="live-portrait row">
          ...
          <div className='text-center col-sm-2'>
            <h5 className = 'score-preview'> 
              {this.props.match_data["radiant_score"]} - {this.props.match_data["dire_score"]} 
            </h5>
            ...
            <h6 className = 'score-preview'>
                {this.props.match_data["match_id"]}
            </h6>
          </div>
          {this.renderHeroIcons(5, 'DIRE')}
          <div className='row-sm-2 p-0 m-0'>
            <LivePlayerNamesView complete = {this.props.match_data['deactivate_time']} 
  ...
  render() {
    return (
      <figure style ={{color:'white'}} className='d-none d-lg-block text-center col-sm-1'>
        <img alt={this.props.player['hero_name']} className='img-fluid' src={'https://d2zromn1qdgyf0.cloudfront.net/heroes_icons/' + this.props.player['hero_id'] + '.png'}/>
      </figure>
      );
  }
}
...
class LivePlayerNamesView extends React.Component {
  ...
  render() {
    ...
    return (
    <p style = {{'color': 'white', 'backgroundColor' : '#515151'}} className='row-sm-2'>
      {this.renderNames()}
      <span className='mmr-spec'> <span style={{'color': color}} > {status} </span> / {this.props.mmr} MMR / {this.props.spectators} Spectators </span>
    </p>
    );
