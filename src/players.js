...
export default class PlayersList extends React.Component {
  ...
  getRoutes() {
    if (this.state.players_list !== null) { 
      ...
      for(var i = 0; i < this.state.players_list.length; i++) {
        rows.push(
        <Route key={this.state.players_list[i]['player_id']} path={'/players/' + this.state.players_list[i]['player_id']}> 
          <Matches 
  ...
  renderPlayerNames() {
    if (this.state.players_list !== null) { 
      ...
      for(var i = 0; i < this.state.players_list.length; i++) {
        rows.push( 
          <PlayerItem key={this.state.players_list[i]['player_id']}
...
class PlayerItem extends React.Component {
  renderSigHeroes() {
    if (this.props.sig_heroes !== null) {
      ...
      for(var i = 0; i < this.props.sig_heroes.length; i++) {
        cols.push(
          <img alt="hero" key={this.props.sig_heroes[i]} src={'https://d2zromn1qdgyf0.cloudfront.net/heroes_mini_icons/' + this.props.sig_heroes[i] + '.png'}/>);
      }
      return cols;
    }
  }
  ...
  render() {
    return (
      <tr>
          <td> <Link style={{'textDecoration': 'none', 'color': 'white'}} to={'/players/' + this.props.player_id}> 
            <span className='player-name'> {this.props.player_name} </span> </Link>  
          </td>
          <td> {this.renderSigHeroes()} </td>
          <td> <span> {this.props.twitch}</span> </td>
      </tr>
      )
  }
